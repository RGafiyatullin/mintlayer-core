...
fn check_integrity(orphans: &TxOrphanPool) {
    ...
    orphans.maps.by_spent_output.iter().for_each(|(outpt, iid)| {
        let inputs = orphans.get_at(*iid).transaction().inputs();
        assert!(
            inputs.iter().any(|i| i.outpoint() == outpt),
...
fn insert_and_delete(#[case] seed: Seed) {
    ...
    assert_eq!(orphans.maps.by_spent_output.len(), n_inputs);
    ...
    assert!(orphans.maps.by_spent_output.is_empty());
    check_integrity(&orphans);
}
